...
- `/admin`: painel administrativo. Exige perfil admin **ou** usuário com pelo menos uma seção administrativa liberada em `adminSectionAccess`. O painel agora é compartimentado: mostra botões de seção e renderiza apenas a seção selecionada. Seções atuais: `Cadastro`, `Membros`, `Registro`, `Dashboard de registros`, `Relatório de visitantes`, `Congregações`, `Usuários`, `Auditoria` e `Site público`. Admin vê tudo; usuários não-admin veem somente as seções liberadas.
  - **Registro** (`AdminPresenceRegistration`): lançamento rápido de presença para visitante/convidado sem conta, gravando em `visitRecords` com `source: 'admin'`.
  - **Dashboard de registros** (`RecordsDashboard`): seção própria para gestão/análise dos registros. Lê `visitRecords` e mostra visitante/convidado, data, horário registrado, culto/atividade, igreja, classificação, origem e convidante. Possui filtros por busca livre, período, tipo, culto/atividade, dia da semana, classificação, igreja e origem; indicadores de total, visitantes, convidados, pessoas únicas, congregações, registros com login e registros lançados pela administração; rankings por tipo, culto, dia, origem, classificação e congregação; últimos lançamentos; tabela detalhada e exportação CSV.
  - **Relatório de visitantes** (`VisitorWorshipReport`): seção própria para leitura nominal no culto. Filtra por data, culto/atividade, igreja e tipo; mostra total de visitantes/convidados, texto pronto para leitura, lista nominal, botão de copiar texto e opção de imprimir. A permissão da seção é `relatorio_visitantes`, com leitura dos registros sem permissão de escrita.
  - **Aprovação de cadastro** (`MembershipApprovals`): seção administrativa própria, separada do menu Cadastro, para ser atribuída de forma independente em `adminSectionAccess` pela chave `aprovacao_cadastros`. Lê `membershipRequests` em tempo real **filtrando só `tipoPessoa === 'membro'`** (visitante/convidado não passam por aprovação), com filtros de status e conferência obrigatória antes de aprovar. Ao aprovar, uma operação em lote cria/atualiza o registro oficial em `members/{requestId}`, marca a solicitação como aprovada e promove para membro a conta vinculada por `userId`. Contas de diretoria/admin preservam o perfil de acesso.
  - **Membros** (`MembersAdminSection`): dentro da seção `Membros`, agora há abas internas **Cadastro de membros** e **Progressão espiritual**. O cadastro oficial (`MemberDirectory`) lista em tempo real a coleção `members`, mostra total de ativos e permite buscar por nome, congregação, e-mail ou telefone. A listagem aparece em formato de tabela numerada, com filtro `Ativos`/`Inativos`/`Todos`. A tabela usa `colgroup` e variáveis CSS (`--member-col-*`) em `src/index.css` para ajuste manual das larguras das colunas; os separadores de coluna são transparentes. Os botões **Excluir cadastro** e **Alterar cadastro** foram compactados. O status do cadastro pode ser alterado diretamente entre `ativo` e `inativo`, sem depender do botão excluir. O botão **Excluir cadastro** continua existindo como atalho para inativar (`status: 'inativo'`, com `deletedAt`/`deletedBy`) e remove da lista ativa, sem apagar definitivamente o histórico. Quando há `userId`, o sistema tenta voltar o acesso para `pendente`. O botão **Alterar cadastro** abre o cadastro completo do membro. O admin pode alterar dados cadastrais no documento oficial `members/{cpf}`; quando há `userId`, o sistema tenta sincronizar também o perfil em `users/{uid}`. O e-mail de acesso aparece bloqueado e não é editado no cadastro. Alteração de status, exclusão/inativação e edição de cadastro geram registros em `auditLogs`.
  - **Cadastro de membros e congregados** (`AdminNominalRegistration`): ferramenta recolhível dentro da seção `Cadastro` para registrar **somente membro ou congregado** sem criar login e senha para a pessoa. Usa `RegistrationForm mode="admin" allowedPersonTypes={['membro', 'congregado']}` e grava em `membershipRequests`. Não aprova cadastros; a aprovação fica na seção independente `Aprovação de cadastro`. Visitantes e convidados ficam fora desta aba e devem ser tratados pela seção de presenças/visitantes.
  ...
  - **Progressão espiritual e cargos** (`ProfileProgressionManager`): agora fica em aba interna da seção `Membros`. Lista visitantes/convidados, congregados e membros. Admin promove visitante/convidado para congregado, congregado para membro informando data de batismo, e atribui/remove cargo somente em perfil de membro. A tela mostra quais dados estão pendentes para cada mudança: ao tornar visitante/convidado em congregado, lista o que precisará ser completado depois; ao promover congregado para membro, lista os campos cadastrais pendentes e a data de batismo; ao atribuir cargo, informa pendências como data de batismo ou descrição de outra função. Cada bloco de pendência possui atalho **Editar cadastro/Resolver pendência**, abrindo o editor completo do perfil logo abaixo da lista para corrigir os dados sem sair da aba.
  ...
  - **Auditoria do sistema** (`AuditLogPanel`): lê a coleção `auditLogs` e mostra quem alterou, quando alterou, qual entidade foi alterada, resumo, campos alterados e detalhes de antes/depois. Atualmente registra alterações administrativas do cadastro oficial de membros.
...
Quando o administrador já cadastrou uma pessoa sem login, o membro/congregado não recebe senha provisória criada pelo administrador. Em `/cadastro`, `CreateAccessPanel` possui duas abas: **Novo cadastro** e **Já fui cadastrado**. Na aba **Já fui cadastrado**, a pessoa informa o e-mail cadastrado e cria a própria senha; o sistema cria o usuário no Firebase Authentication com `pendingFirstAccess: true` e envia confirmação por e-mail. Após confirmar o e-mail, `FirstAccessVerificationPanel` permite clicar em **Já confirmei meu e-mail**; então `claimExistingRegistrationByEmail` procura cadastro correspondente em `members` e `membershipRequests`, vincula o `userId` ao documento existente e atualiza `users/{uid}` com os dados cadastrais e o perfil de acesso correspondente. As regras do Firestore permitem esse vínculo apenas quando `emailLower` do cadastro coincide com o e-mail autenticado e somente nos campos de vínculo (`userId`, `linkedBy`, `linkedAt`, `updatedAt`).
...
- Dados exigidos no formulário de congregado: nome, CPF, data de nascimento, sexo, telefone, WhatsApp, e-mail, congregação, endereço completo, data de aceitação, documentos e LGPD. RG é opcional.
...
- Nome completo, CPF (validado com dígito verificador), Data de nascimento, Sexo, Congregação (dica "que frequenta"), Telefone (com marcar "tem WhatsApp"), E-mail do acesso. RG é opcional, preenchido somente com números, e possui campo separado **UF do RG** em lista de UFs.
...
- Endereço: CEP (com busca automática ViaCEP), País (pré-preenchido como Brasil), UF do endereço em lista de estados, município em lista pesquisável, bairro em lista pesquisável, Tipo de logradouro (lista: Rua/Avenida/Alameda/...), Nome do logradouro, Número, Complemento (opcional). Ao informar CEP válido, o sistema preenche país, UF, município, bairro, tipo de logradouro e nome do logradouro quando o ViaCEP retornar esses dados; ficam pendentes número e complemento.
...
- O painel do membro (`/membro`) agora exibe o editor **Meu cadastro**, permitindo que o usuário logado altere seus dados cadastrais no próprio perfil `users/{uid}`. O e-mail de acesso é somente leitura. Para troca de e-mail, o usuário usa a ferramenta **Trocar e-mail de acesso**, que chama `verifyBeforeUpdateEmail` do Firebase e envia link de confirmação ao novo endereço; após confirmar e entrar novamente, o sistema sincroniza `users/{uid}.email`/`emailLower` com o e-mail confirmado do Authentication.
...
A versão vigente no repositório (com `adminSectionAccess` / `hasAdminSection` liberando operações por seção — `site`, `congregacoes`, `cadastros`, `aprovacao_cadastros`, `presencas`, `dashboard_registros`, `relatorio_visitantes`, `membros`, `usuarios`, `auditoria`) precisa estar publicada no console para usuários não-admin acessarem as seções compartimentadas. Quando `firestore.rules` for alterado novamente, substituir todo o conteúdo na aba **Firestore Database → Regras** e clicar em **Publicar**.
