...
- CPF é a chave principal da pessoa no cadastro oficial. Novos registros salvam também `cpfDigits` (somente números) e, ao aprovar membro, o documento em `members` passa a usar o CPF numérico como ID. No cadastro administrativo de membro/congregado, antes de gravar, o sistema consulta `members`, `membershipRequests` e `users`; se o CPF já existir, bloqueia a criação e orienta o administrador a editar o cadastro existente com aviso logo abaixo do campo CPF e também no fim do formulário.
